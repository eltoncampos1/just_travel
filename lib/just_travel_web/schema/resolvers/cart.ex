defmodule JustTravelWeb.Schema.Resolvers.Cart do
  ...
  def create(_, _ctx) do
    ...
    with {:ok, %Cart.Commands.CreateCart{} = create_command} <-
           Utils.Changesets.cast_and_apply(Cart.Commands.CreateCart, %{cart_id: cart_id}),
  ...
  def get_by_id(%{cart_id: cart_id} = params, _ctx) when not is_nil(cart_id) do
    with {:ok, %Cart.Commands.GetCartById{} = get_cart} <-
           Utils.Changesets.cast_and_apply(Cart.Commands.GetCartById, params),
